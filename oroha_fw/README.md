...
Sheet A 구성(전류센서 ×2 @3.3 V + Raspberry Pi Pico) 전용.

> ⚠ **실장 전류센서는 `ACS37030LLZATR-020B3`** 이다 — ±20 A, 3.3 V, 66 mV/A, **비-비율**.
> 도면 파일명과 설계 문서의 "CJMCU-758 / ACS758" 표기는 실물과 다르다. 2026-08-14 DMM
> 15 점 교정으로 확정했다([보고서 §7](../docs/hardware_test_20260814.md)) — 그 전까지
> 펌웨어는 26.4 mV/A(2.54 배 과대), `test/` 스크립트는 다른 값(18.8% 과소)을 쓰고 있었다.
...
**목표 < 30 mA.** 1 LSB가 12.03 mA(2026-08-14 교정)이므로 목표는 약 2.5 LSB에 해당한다. σ가 1 LSB 이하면 양자화가 지배한다는 뜻이고, 그보다 크면 배선·접지·EMI를 의심한다.
...
| 2 | 스트리밍 켜고 raw가 **≈2040** (0 A 기대값, §6 참조) 근처인지, overrun 없는지 | `monitor`, `G` |
...
## 5. 전류센서 — ACS37030 (이미 이 부품이 달려 있다)

2026-08-14 교정 전까지 이 절은 "나중에 바꿀 때" 안내였다. **실물이 이미 ACS37030이다.**
아래 표의 오른쪽 열이 현재 상태다.


| 항목 | ACS758 (문서가 가정하던 것) | **ACS37030 (실물)** |
|---|---|---|
| 헤더 순서 | `VCC/GND/OU1/OU2` | `GND/VREF/VOUT/VDD/GND/GND` — **완전히 다름, 재배선 필수** |
| 감도 | 26.4 mV/A | **66 mV/A → `a_per_lsb` 12.21e-3** (실측 교정 12.029e-3, `scale_gp28` 0.9852) |
...
**구현됨** (2026-08-14, 조치 #18). `calibrate_zero()`가 영점을 잰 뒤 위 식으로 레일을 역산하고,
교정 시점(`V_RAIL_REF = 3.27605 V`) 대비 보정 계수 `rail_corr`를 만들어 **전류와 전압 환산에
모두** 곱한다. 비-비율 센서는 게인도 레일에 비례하기 때문이다. `#CFG`와 `#ZERO`에 `rail=` /
`rail_corr=`로 나가고, ROS2 노드는 `#ZERO`에서 받아 `rail_corr` 파라미터로 반영한다.

주의할 점 둘:

- **정지 영점에는 컨트롤러 대기전류가 실려 있다.** 채널별 상수(`QUIET_GP28` / `QUIET_GP27`)로
  명시적으로 빼고 평균한다. 지금은 `SIGN_GP27 = -1` 이라 두 채널에서 대기분이 서로 상쇄돼
  (0.03 LSB) 대기전류가 변해도 자동으로 상쇄되지만, **조치 #20으로 센서 #2 배선을 고치면
  그 상쇄가 사라진다** — 그때 `QUIET_GP27` 부호를 뒤집고 값을 다시 재야 한다.
  > **✅ 2026-09-03 갱신 — 이미 일어난 일이다.** 센서 #2 의 IP 역결선은 08-15~08-21
  > 사이에 교정됐고, 08-28 절대 교정이 `SIGN_GP27 = +1` 을 확정했다 (`pico/main.py`
  > 의 부호 블록 · `docs/sensing_constants.md`). 위 문단은 **`SIGN_GP27 = -1` 이던
  > 시절의 서술**이라 그대로 두되, 지금 상태는 상쇄가 **없는** 쪽이다 —
  > `QUIET_GP28` / `QUIET_GP27` 이 각각 7.0 LSB 로 잡혀 있고 둘 다 양수다.
- 이 식은 센서 무전류 출력이 **정확히 1.65 V**라고 가정한다. 부품 자체 영점 오차는 레일 오차로
  둔갑한다. 2026-08-14에 두 채널이 3.2790 / 3.2731 V로 0.18% 안에서 일치했으므로 그 오차는
  작다고 보지만, **T-1c로 DMM 실측 VCC와 대조해 검증할 것.**
...
| 영점이 기대값에서 크게 벗어남 | **VCC를 같이 재라 — 다만 이유가 반대다.** ACS37030은 **비-비율**이라 무전류 출력이 1.634 V로 고정이고, raw는 `1.634 / VREF × 4095`이므로 **VREF가 오르면 raw는 내려간다.** VREF 3.280 V에서 기대값은 **2048이 아니라 ≈2040**이고, 실제 교정된 0 A raw는 gp28 2033.97 · gp27 2035.26이다 (`docs/sensing_constants.md`). VREF로 설명되지 않는 이동만 배선·접지 문제다 |
