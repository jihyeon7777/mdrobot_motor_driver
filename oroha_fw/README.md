...
지금 펌웨어에는 이 로직이 **아직 없다** (조치 #18). 2026-08-14 실측 `raw_zero = 2060.63`을
위 식에 넣으면 `V_rail ≈ 3.279 V`로, 공칭 3.3보다 0.6% 낮다. 게인 쪽에서 역산하면 3.251 V가
나와 0.9%p 벌어지는데 이 차이는 아직 설명되지 않았다. `calibrate_zero()`에 추가하고
**T-1c**로 DMM 실측 VCC와 대조해 검증할 것.
