...
DMM 설정
  - ⚠ **레인지는 리그 구성에 따라 다르다. 아래 숫자를 확인하고 고를 것.**

        바퀴·벨트 없음 (~08-21)          최대 약 0.5 A
        바퀴·벨트 장착 (08-26~), 1500 rpm  약 0.9 A
        바퀴·벨트 장착, **3000 rpm**       **약 1.1~1.4 A**   ← 2 A 레인지가 필요하다

    아랫줄은 20260826 `bi4` 로그 실측이다 (채널당 1.319/1.355 A, 23.1 V). 버스가 높으면
    같은 마찰에 전류는 반비례로 준다 — 28.5 V 에서는 1.1 A 대다.
  - **레인지를 넘지 않는 선에서 자릿수가 가장 많은 레인지**를 쓸 것.
  - ⚠ **mA 잭에 꽂지 말 것 — 퓨즈가 나간다.** 션트 저항이 커서(수 Ω) 전압강하로
    컨트롤러도 방해한다. 위 최대치를 감당하면서 버든 전압이 작은 레인지를 고를 것.
  - 읽은 자릿수를 **그대로** 입력할 것. 반올림하면 그만큼 교정에 실린다.

한계 (미리 알고 시작할 것)
  ⚠ 아래는 **무부하(바퀴 없음) 리그** 기준이며 08-14 교정이 그 조건이었다. 바퀴·벨트를
  달고 3000 rpm 까지 쓰면 raw 스팬이 110 LSB 를 넘어 이 제약은 크게 완화된다.
  무부하 전류는 0.05~0.48 A 로 **±20 A** 센서의 맨 아래 2.4% 구간이다. raw 스팬이 14 LSB 뿐이라
