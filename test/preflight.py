...
# 2026-08-28 DMM 직렬 절대 교정으로 확정 — 실효 A/LSB 와 0 A 인 raw.
#   **단일 출처는 docs/sensing_constants.md 다.** 게인은 양 채널 공통 11.44 mA/LSB
#   (감도 70.0 mV/A) 이고 **둘 다 양(+)** 이다. 08-14 규약의 GP27 음부호는 센서 #2 IP
#   역결선 때문이었는데 08-15~08-21 사이에 배선이 교정돼 무효가 됐다 (조치 #35).
#   0 A raw 는 교정 회귀의 x 절편이다 — 스위치 OFF 실측보다 신뢰도가 높다.
CAL = {"gp28": (11.44e-3, 2033.97), "gp27": (11.44e-3, 2035.26)}
QUIET_A = 0.080          # DMM 실측 컨트롤러 대기전류 [A] — 양 채널 동일 (7.0 LSB)
VOLT_GAP = 0.600         # id2 − id1, 6 세션 연속 재현된 계측 오프셋 [V]

# 통전 정지 raw 의 기준 대역 — 2026-08-28 갱신 (조치 #42 종결).
#   옛 값(gp27 2052.9~2053.4 / gp28 2068.0~2070.1)은 8/11·8/12 것이라 매 세션 주의 2 건을
#   띄웠다. 08-27 §4 가 그 간극을 드리프트가 아니라 **배선 변경 계단**으로 규정했고,
#   08-28 절대 교정이 참 0 A 를 확정하면서 기준을 다시 잡을 근거가 생겼다.
#   아래는 08-28 의 네 관측을 덮는 대역이다:
#     교정 정지점 gp28 2040.63 / gp27 2042.20   ·  냉간 zero_on gp28 2040.60 / gp27 2042.72
#     교정 직전   gp28 2039.62 / gp27 2042.85   ·  flash 후    gp28 2039.97 / gp27 2043.29
#   ⚠ 참 0 A 는 하루 ±2 LSB 흔들린다 (#27). 본 시험은 **로컬 영점**을 쓰므로 결과에는
#   영향이 없고, 이 대역은 "무언가 크게 달라졌나"를 보는 용도다.
...
# ⚠ b 는 호스트·배선에 묶인다. 노트북은 Δ = 2.1 mV 다 (20260821 sensing §1). 호스트를
# 바꾸면 핀 35(VREF)와 핀 33(Δ)을 함께 다시 잴 것. 옛 상수(b=0, 8.9815 mV)와의 실제
# 차이는 작동 범위에서 최대 35 mV 다 — 바꾼 이유는 정확도가 아니라 물리적 분해다.
VOLT_GP26_B = -19.60
