...
# ⚠⚠ **이 값은 틀렸다 (2026-09-05).** "30 counts/모터축 × 감속기 30:1" 로 세운 값인데,
#   감속기 출력과 바퀴 사이에 **타이밍벨트단(모터측 22 T : 바퀴측 17 T)** 이 하나 더
#   있다. 이 저장소의 어느 파일도 그것을 몰랐다.
#
#   토크를 끄고 6.000 m 줄자 구간을 손으로 밀어(견인력 0 = 미끄러짐 0) 직접 쟀다:
#
#       실측 **0.7613 mm/count**  vs  여기서 쓰는 0.8867 (= 798/900)   → **−14.1%**
#
#   ⚠ 아직 고치지 않는다. 250 mm 타이어에 맞는 counts/바퀴회전은 약 **1031** 인데
#     30:1(900) 도 30×17/22(696) 도 30×22/17(1165) 도 아니다. 의심은
#     "30 counts/모터축" 쪽이고 그 값은 이 저장소에서 측정된 적이 없다
#     (CLAUDE.md: "하드코딩하지 않는다 — 측정한다"). **타이어 3 바퀴 측정**이
#     벨트 방향과 counts/모터축을 동시에 결정한다. 그 전에 값을 바꾸면 두 번 틀린다.
#
#   여기(화면 표시)와 `WHEEL_CIRC` 는 **비로만** 쓰이므로 둘을 함께 고쳐야 하고,
#   분석은 이미 실측 0.7613 mm/count 로 다시 냈다 (20260905 §4·§6).
