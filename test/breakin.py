...
# 채널별 실효 A/LSB — 2026-08-28 DMM 직렬 절대 교정으로 **확정**. 이 시험은 종료됐다.
#   두 채널 공통 **11.44 mA/LSB** (센서 감도 70.0 mV/A @ VREF 3.280 V)
#
#   ⚠ 채널별 적합은 GP28 11.4611 / GP27 11.4137 을 냈으나 그 0.42% 차이는 **채택하지 않는다.**
#     같은 掃引의 상승/하강 폭이 −1.76% / +2.13% 라 채널 차보다 4 배 크다 — 구별할 수 없는
#     차이를 상수에 새기면 없는 정밀도를 주장하게 된다. 단일 게인으로 17 점 × 2 채널을 다시
#     맞춰도 잔차 RMS 는 16.4 / 10.5 mA 로 채널별 적합(16.4 / 10.4)과 같다. 잃는 것이 없다.
#   실질 불확실도 **±1.5%** (掃引 폭). 절대 전력은 이 이상으로 믿지 말 것.
#
#   ⚠ 두 채널 모두 **양(+)** 이다. 08-14 규약의 GP27 음부호(센서 #2 IP 역결선)는 실재했으나
#     08-15~08-21 사이에 배선이 교정되면서 무효가 됐다 — 20260821_sensing §4 가 진단했고
#     (조치 #35) 08-28 교정이 부호와 크기를 함께 확정했다.
#   게인비 실측 1.0042 는 08-21 스왑 시험 1.0022 와 0.20% 일치 — 둘 다 1.00 으로 본다.
LSB_A_CH = {"gp27": 11.44e-3, "gp28": 11.44e-3}
...
#   호스트나 배선을 바꾸면 **핀 35(VREF)와 핀 33(Δ)을 함께 다시 재고** 이 값을 갱신할 것.
#
#   다만 **옛 상수(b = 0, 8.9815 mV)와의 실제 차이는 작다** — 그 모델이 0 이 아닌 절편을
#   기울기에 흡수하고 있었기 때문이다. 작동 범위 20.5~28.7 V 에서 최대 35 mV 이고,
#   08-21 노트북 점(raw 2893.888, DMM 25.99)에 이 상수를 적용해도 −13 mV 다.
#   바꾼 이유는 정확도가 아니라 **분해**다: 기울기는 분압기, 절편은 접지 오프셋으로
#   물리량이 나뉘어야 호스트가 바뀌었을 때 어느 항을 다시 재야 하는지 알 수 있다.
# 2026-08-28 확정. Δ 실측 15.7 mV(08-26) / 13.8 mV(08-28) → **15.0 mV** 로 확정하고,
# 그 절편을 고정한 채 DMM 3 점에서 D 를 풀면 11.1298~11.1383 (폭 0.077%) → **D = 11.130**.
# 세 점 잔차 −15 / −18 / +1 mV (≤0.08%). 옛 상수 대비 전 구간 −16 mV (−0.07%) 로 실용 차이 없다.
GP26_B_LSB = -18.7       # 접지 오프셋 절편 [LSB] — Δ 15.0 mV / VREF 3.280 V
V_PER_LSB = 8.913e-3     # (raw − GP26_B_LSB) 에 곱한다.  = LSB_ADC 0.800781 mV × D 11.130
...
def reanalyze(tag: str, dmm: float | None) -> int:
    ...
    print(f"  ⚠ v_bus 는 파이 + 2026-08-26 배선 상수 (raw {GP26_B_LSB:+.1f}) × "
          f"{V_PER_LSB * 1e3:.4f} mV 로 환산한다. 다른 호스트·배선에서 딴 로그에는\n"
          f"    접지 오프셋이 다르다 — 실제 차이는 작지만(작동 범위에서 최대 35 mV) "
          f"엄밀히는 그 세션의 Δ 로 환산해야 한다. 전류·비 는 영향 없다.")
